...
int main( int argc, char **argv )
{
    ...
    window = glutCreateWindow( "TestShader");
...
//    readShaderSource("phong", &VertexShaderSource, &FragmentShaderSource);
//    success = installPhongShaders(VertexShaderSource, FragmentShaderSource);
	...
	{
		readShaderSource("fixed", &VertexShaderSource, &FragmentShaderSource);
		success = installShaders(VertexShaderSource, FragmentShaderSource);
		success *= initFixedFuncShader();
	}
	else if(example == 2)
	{
		readShaderSource("phong", &VertexShaderSource, &FragmentShaderSource);
